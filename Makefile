...
release: build
	docker tag -f convox/agent:latest convox/agent:0.7
	docker push convox/agent:0.7
	AWS_DEFAULT_PROFILE=release aws s3 cp convox.conf s3://convox/agent/0.7/convox.conf --acl public-read
